layui.use(['table','form','laydate'], function(){
    ...
    var cols = [[
        ...
        ,{field:'phone', title:'电话', width:200, edit:'text'}
    ...
    tableData('/back/customer/query',null,cols);
    // 模糊查询按钮监听
    form.on('submit(sreach)', function(data){
        data = data.field;
        tableData('/back/customer/findAllByLike',data,cols);
        return false;
    })
    //触发单元格工具事件
    table.on('tool(test)', function (obj) { // 双击 toolDouble
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                // 删除数据库数据
                let myAjax1 = myAjax('/back/customer/del/'+data.id, null,'DELETE');
            ...
            xadmin.open('简介修改','/html/customer/customer-edit.html',600,400);
        }
    });
});
