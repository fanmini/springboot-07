layui.use(['table','form','laydate'], function(){
    ...
    function findAll(data){
        table.render({
            elem: '#test'
            ,url:'http://localhost:8080/back/ourTeam/query'
    ...
    table.on('tool(test)', function (obj) { // 双击 toolDouble
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                // 删除数据库数据
                let myAjax1 = myAjax('http://localhost:8080/back/ourTeam/del', {id:data.id},'post');
            ...
            sessionStorage.setItem('coachId',data.id);
