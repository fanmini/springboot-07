layui.use(['form', 'layedit','layer', 'laydate','upload'], function () {
    ...
    // 渲染模糊类型
    let nav = myAjax("http://localhost:8080/back/product/productNav", null, "get");
    setSelect(nav);
    findById();
    ...
    function findById(){
        let id = sessionStorage.getItem("productId");
         let res = myAjax("http://localhost:8080/back/product/query",{id:id},"get");
         if(res!=null) {
    ...
    form.on('submit(edit)',function (data){
        ...
        let res = myAjax("http://localhost:8080/back/product/set",data,'post');
        if(res.count>0){
            layer.alert(
                '修改成功',
                {icon:6},
